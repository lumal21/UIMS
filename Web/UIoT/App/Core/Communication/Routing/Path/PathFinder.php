...
final class PathFinder
{
	...
	public function mountRouter(IRouter $router)
	{
		...
		array_map(function ($node) use ($router) { $this->mountRouterCoreNode($router, $node); }, $this->getNodeIndexer()->getNodesByPriorityId($this->getNodeIndexer()->getLowestNodePriorityId()));
	}
	...
	public function mountRouterNode(IRouter $router, NodeModel $node)
	{
		...
		$router->mount($node->getPath(), function () use ($router, $node) { $this->mountRouterEdges($router, $node); });
	}
	...
	public function serializeCallBacks(array $nodes)
	{
		...
		array_walk_recursive($nodes, function (NodeModel $edge) { $edge->getCallback()->setNodeModel($edge); });
	}
}
